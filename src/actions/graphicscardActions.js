...
export const getGraphicsCardList = () => async (dispatch) => {
    try {
        ...
        // call api
        const { data } = await axios.get("http://102.212.245.33/api/graphicscard/", {
            method: 'POST',
            mode: 'cors',
            headers:{
...
export const getGraphicsCardDetails = (id) => async (dispatch) =>{
    try {
        ...
        // call api
        const { data } = await axios.get(`http://102.212.245.33/api/graphicscard/${id}/`)
        dispatch({
...
export const creategraphicscard = (graphicscard) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.post(
            "http://102.212.245.33/api/graphicscard-create/",
            graphicscard,
            config
        )
...
export const deletegraphicscard = (id) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.delete(
            `http://102.212.245.33/api/graphicscard-delete/${id}/`,
            config
        )
...
export const updategraphicscard = (id, graphicscard) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.put(
            `http://102.212.245.33/api/graphicscard-update/${id}/`,
            graphicscard,
            config
        )
